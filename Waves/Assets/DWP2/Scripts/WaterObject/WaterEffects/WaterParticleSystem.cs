...
namespace DWP2.WaterEffects
{
    ...
    public class WaterParticleSystem : MonoBehaviour {
        ...
        public int TriDataLength => _targetWaterObject.TriDataLength;
        public int TriDataStart => _targetWaterObject.TriDataStart;
        public int TriDataEnd => _targetWaterObject.TriDataEnd;

        public int VertDataLength => _targetWaterObject.VertDataLength;
        public int VertDataStart => _targetWaterObject.VertDataStart;
        public int VertDataEnd => _targetWaterObject.VertDataEnd;


        void Start()
        {
            _targetWaterObject = GetComponentInParent<WaterObject>();
            if (_targetWaterObject == null)
            {
                Debug.LogError($"{name}: WaterParticleSystem requires WaterObject attached to the same object or one of parent objects to function.");
                return;
            }
        ...
        void LateUpdate()
        {
            if (!_initialized || !_targetWaterObject.Initialized) return;
            if (!emit) return;
        ...
        private void EmitNew()
        {
            ...
            WaterObjectManager waterObjectManager = WaterObjectManager.Instance;
            ...
            if (emit && _timeElapsed >= emitTimeInterval && TriDataLength > 0f)
            {
                ...
                {
                    elevation = WaterObjectManager.Instance.GetWaterHeight(Vector3.zero);
                }
                else
                {
                ...
                {
                    //if (waterObjectManager.States[i] != 1) continue;
                ...
                while (emitted < emitPerCycle)
                {
                    ...
                    int n = waterObjectManager.P0S.Length / 3;
                    
                    EmitParticle(
                        waterObjectManager.P0S[waterLineTriIndex * 3 + 2], 
                        waterObjectManager.P0S[waterLineTriIndex * 3 + 1], 
                        elevation, 
                        waterObjectManager.Velocities[waterLineTriIndex], 
                        waterObjectManager.Normals[waterLineTriIndex], 
                        waterObjectManager.Forces[waterLineTriIndex], 
                        waterObjectManager.Areas[waterLineTriIndex]);
